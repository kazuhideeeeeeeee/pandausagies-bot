...
def generate_ai_tweet(mode: str, image_context: Optional[str] = None) -> str:
    ...
    image_context:
        画像から読み取った「人数・場所・空気感」などの説明（あれば）
    """

    # それ以外が来たらとりあえず daily に寄せる
...
- 朝・昼・夜など時間帯が決まってしまう言い回し（「おはよう」「朝から」「今夜は」など）は使わない。
- 長文にしない。1〜2文。
...
"""
    ...
    text = "".join(text.split())  # 改行・余分な空白を削る
