...
class SupabaseStorage:
    ...
    def health(self) -> dict[str,Any]:
        ...
        return {"environment":"staging","storage":"OK","memory":"OK","posts":len(memory.posts),"current_week":memory.weeks[-1].get("id") if memory.weeks else "week-00 preview","open_events":sum(e.get("status")=="open" for e in memory.events),"last_run":last[0]["started_at"] if last else None,"last_success":success[0]["finished_at"] if success else None,"last_decision":last[0].get("decision") if last else None,"consecutive_errors":self.get_setting("consecutive_errors",0),"circuit_breaker":"OPEN" if self.get_setting("circuit_open",False) else "CLOSED"}
